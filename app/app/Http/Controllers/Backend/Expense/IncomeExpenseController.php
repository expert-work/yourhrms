...
class IncomeExpenseController extends Controller
{
    ...
    {
        try {
            $data['title'] = __translate('Income/Expense List');
    ...
    public function store(IncomeExpenseRequest $request)
    {
        ...
        public function update(IncomeExpenseRequest $request, $id)
        {
            try{
                ...
                if ($data) {
                    Toastr::success(_translate('Category has been updated successfully.'), 'Success');
                    return redirect()->route('income_expense.index');
                } else {
                    Toastr::error(_translate('Something went wrong!'), 'Error');
                    return redirect()->back();
        ...
        public function destroy($id)
        {
            try {
                ...
                if ($data) {
                    Toastr::success(_translate('Income-Expense has been deleted successfully.'), 'Success');
                    return redirect()->route('income_expense.index');
                } else {
                    Toastr::error(_translate('Something went wrong!'), 'Error');
                    return redirect()->back();
