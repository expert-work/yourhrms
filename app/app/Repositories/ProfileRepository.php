...
class ProfileRepository
{
    ...
    public function __construct(User $user, CompanyRepository $companyRepo , UserRepository $userRepo, DepartmentRepository $department,TaskService $tasksService)
    {
        $this->user = $user;
        ...
        return datatables()->of($drivers)
            ->addColumn('action', function ($data) {
                $button = '<div class="flex-nowrap">
                    <div class="dropdown">
                        <button class="btn btn-white dropdown-toggle align-text-top" data-boundary="viewport" data-toggle="dropdown"> ' . __translate('Action', false) . '</button>
            ...
            ->addColumn('drivers', function ($data) {
                ...
                if ($getChildDrivers->count() > 1) {
                    ...
                    return '<span class="badge badge-danger" > ' . __translate('No Driver Assign', false) . ' </span > ';
                }
            })
    ...
    public function getProfile($request, $slug = 'official'): \Illuminate\Http\JsonResponse
    {
        ...
        if ($user) {
            if ($slug === 'official') {
                ...
                return $this->getSecurityInfo($user, $slug);
            ...
            elseif ($slug === 'attendance') {
                return $this->getAttendanceInfo($user, $slug);
            }
            elseif ($slug === 'tasks') {
    ...
    public function getSecurityInfo($user, $slug): \Illuminate\Http\JsonResponse
    {
    ...
    public function deleteFaceData($request)
    {
        $user = auth()->user();
        $user->face_data = null;
        $user->save();
        return $this->responseWithSuccess('Face Recognition Data Cleared', [], 200);
    }
