...
class ExpenseRepository
{
    ...
    function UserExpenseList($request)
    {
        ...
        if($request->date ==null && $request->status == null && $request->payment == null) {
            $content = $content->latest()->take(10)->get();
        }else{
    ...
    public function datatable($request)
    {
        ...
        return datatables()->of($content->latest()->get())
            ->addColumn('action', function ($data) {
                ...
                // if (hasPermission('expense_invoice')) {
                //     $action_button .= '<a href="' . route('hrm.expenses.invoice', $data->id) . '" class="dropdown-item"> ' . _trans('common.Invoice') . '</a>';
                // }
