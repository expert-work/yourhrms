                ...
                <div class="row p-5">
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" min="0" step=any name="subject"
                                id="subject" autocomplete="off" required onkeyup="discussionValidate()" />
                            <div class="error_show_subject"></div>
                        </div>
                        <div class="form-group">
                            <label class="cus-label">{{ _trans('common.Description') }}</label>
                            <textarea type="text" name="description" id="description" rows="5" class="form-control summernote" required
                                onkeyup="discussionValidate()">{{ old('description') }}</textarea>
                            <div class="error_show_description"></div>
                        </div>
                        <div class="form-group d-none">
                            <div class="checkbox checkbox-primary">
                                <input type="checkbox" name="show_to_customer" value="1" checked=""
                                    id="show_to_customer" onclick="discussionValidate()">
                                <label for="show_to_customer">{{ _trans('project.Visible to Customer') }}</label>
                            </div>
                        </div>
                        <div class="form-group text-right">
                            <button type="button" class="btn btn-primary pull-right"
                                onclick="submit_discussion()">{{ _trans('Submit') }}</button>
                        </div>
