    ...
    <div class="card card-primary card-outline">
        ...
        <div class="ltn__tab-menu-list">
            <div class="nav">
                ...
                <a class="{{menu_active_by_route('clients.report')}}" href="{{ route('clients.report',encrypt($data['user']->id)) }}">{{_translate('Reports')}}
                    <i class="fas fa-file-alt"></i></a>
                <a class="{{menu_active_by_route('clients.sticker')}}" href="{{ route('clients.sticker',encrypt($data['user']->id)) }}">{{_translate('Sticker List')}} <i
                        class="fas fa-id-card"></i></a>

                <a class="{{menu_active_by_route('report.driver.liveTracking') }}" href="{{ route('report.driver.liveTracking',encrypt($data['user']->id)) }}">{{_translate('Live Tracking')}}
                    <i class="fas fa-search-location"></i></a>
                {{-- <a class="{{menu_active_by_route('clients.sticker_assign_list')}}" href="{{ route('clients.sticker_assign_list',encrypt($data['user']->id)) }}">{{_translate('Sticker Assign List')}} <i
                    class="fas fa-id-card"></i></a> --}}
                <a class="{{menu_active_by_route('client.payment')}}" href="{{ route('client.payment',encrypt($data['user']->id)) }}">{{_translate('Payment')}}
                    <i class="fas fa-money-bill-wave"></i></a>
                <a class="{{menu_active_by_route('client.accountSetting')}}" href="{{ route('client.accountSetting',encrypt($data['user']->id)) }}">{{_translate('Settings')}}<i
                        class="fas fa-user-cog"></i></a>

                <a href="{{ route('client.loginAsClient',encrypt($data['user']->id)) }}">
                    {{_translate('Login as  Client')}}
