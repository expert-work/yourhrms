                ...
                <div class="row p-2">
                    <div class="col-md-12">
                        <form action="{{ $data['url'] }}" method="post">
                            ...
                            {{-- @dd($data['competence_types']) --}}
                            <div class="form-group">
                                <label class="cus-label">{{ _trans('performance.Competence Type') }}</label>
                                <select name="competence_type_id" class="form-control select2" required>
                                    @foreach ($data['competence_types'] as $competence_type)
                                    <option {{ @$data['edit']->competence_type_id == $competence_type->id ? 'selected' :
                                        '' }}
                            ...
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control select2" required>
                                    <option value="1" {{ @$data['edit'] ? ($data['edit']->status_id == 1 ? 'Selected' :
                                        '') : '' }}>
                                        {{ _trans('payroll.Active') }}</option>
                                    <option value="4" {{ @$data['edit'] ? ($data['edit']->status_id == 4 ? 'Selected' :
                                        '') : '' }}>
