...
class CreateProjectsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('projects', function (Blueprint $table) {
            ...
            $table->double('paid',16,2)->nullable()->default(0.00);
            $table->double('due',16,2)->nullable()->default(0.00);

            $table->foreignId('created_by')->index('created_by')->default(1)->constrained('users');

            // notifying all users when project is created
            $table->tinyInteger('notify_all_users')->default(0)->nullable()->comment('0=no,1=yes');

            // notifying all users via email when project is created

            $table->tinyInteger('notify_all_users_email')->default(0)->nullable()->comment('0=no,1=yes');
